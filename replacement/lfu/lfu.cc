...
lfu::lfu(CACHE* cache, long sets, long ways) : replacement(cache), NUM_WAY(ways), freqs(static_cast<std::size_t>(sets * ways), 0) {}
...
{
  auto begin = std::next(std::begin(freqs), set * NUM_WAY);
...
void lfu::replacement_cache_fill(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr,
                                 access_type type)
{
  freqs.at((std::size_t)(set * NUM_WAY + way)) = 0;
}
                                   ...
                                   champsim::address victim_addr, access_type type, uint8_t hit)
{
  if (hit && access_type{type} != access_type::WRITE) // Skip this for writeback hits
    ++freqs.at((std::size_t)(set * NUM_WAY + way));
}
